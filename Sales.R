...
# Define UI
ui <- fluidPage(
  theme = shinytheme("flatly"),  # Change the theme to a purple color scheme
  tags$head(
    tags$style(HTML("
      h3 {
        color: #024F55; //
      ...
      .input-group-addon[for='vardipen'] h3,
      .input-group-addon[for='varindepen'] h3 {
        color: #1c3150 !important; /* Purple color code */
      }
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("vardipen", label = h3("Dependent"),
                  choices = list("y" = "y"), selected = 1),
      
      selectInput("varindepen", label = h3("Independent"),
                  choices = list("x" = "x1","x2","x3","x4","x5"), selected = 1)
    ),
    mainPanel(
      tabsetPanel(type = "tabs",
                  tabPanel("Scatterplot", plotOutput("scatterplot")),
                  tabPanel("Summary Model", verbatimTextOutput("summary")),
